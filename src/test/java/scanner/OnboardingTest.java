...
public class OnboardingTest {
    ...
    void test02_LanguageScreen() throws Exception {
        ...
        // "Grow With Us" shows inside LanguageActivity before transitioning; accept that too
        Assertions.assertTrue(
            act.contains("Language") || act.contains("HelpUs") || act.contains("NewOnBoarding"),
            "Expected Language/GrowWithUs or Onboarding. Got: " + act);
        log("✅ PASS");
    }

    @Test @Order(3)
    void test03_GrowWithUs() throws Exception {
        log("=== TEST 3 — Grow With Us screen ===");
        // "Grow With Us" is shown inside LanguageActivity (fragment/overlay), not a separate activity
        Assertions.assertTrue(isTextPresent("Grow With Us", 10),
            "Expected 'Grow With Us' text on screen. Activity: " + act());

        // Continue button = btnNext (same ID as language Next, text "Continue"), bounds center (540, 2010)
        log("  Clicking Continue (id/btnNext)");
        if (!clickById(PKG + ":id/btnNext", 5)) {
        ...
        String act = act();
        log("After Grow With Us Continue: " + act);
        Assertions.assertTrue(
            act.contains("NewOnBoarding") || act.contains("AdActivity") || act.contains("HelpUs"),
